...
**Subject**: `[Auto-QA] {verdict} - {bug_title}`
...
<table>
  <tr><td>Bug ID:</td><td>#{bug_id}</td></tr>
  <tr><td>Status:</td><td>{verdict}</td></tr>
  <tr><td>Severity:</td><td>{severity}</td></tr>
  <tr><td>Reported:</td><td>{reported_at}</td></tr>
  <tr><td>Analysis Time:</td><td>{total_time}s</td></tr>
</table>

<h3>Root Cause</h3>
<p>{root_cause.explanation}</p>
<pre><code>{root_cause.file_path}:{root_cause.line_number}
{root_cause.code_context}
</code></pre>

<h3>Proposed Fix</h3>
<pre><code>{proposed_fix.to_diff()}</code></pre>

<h3>Test Results</h3>
<ul>
  <li>All tests: {tests_passed}</li>
  <li>Regressions: {regressions_detected}</li>
  <li>Reproducible: {reproducible}</li>
</ul>

<h3>Attachments</h3>
<ul>
  <li><a href="{screenshot_1}">Step 1 Screenshot</a></li>
  <li><a href="{screenshot_2}">Step 2 Screenshot</a></li>
  <li><a href="{fix_diff}">Full Diff</a></li>
</ul>

<hr>
<p><em>Generated by Self-Learning QA Framework</em></p>
```
